<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h1 class="font-black text-3xl text-slate-900 italic">Calendario de Citas</h1>
                <p class="text-slate-500 text-sm mt-1">Visualiza tus citas y reserva haciendo clic en un día disponible.</p>
            </div>
    ...
    @push('styles')
        ...
        <style>
            #calendario-citas { --fc-border-color: #e2e8f0; --fc-today-bg-color: #eef2ff; }
            #calendario-citas .fc-toolbar-title { font-weight: 900; font-style: italic; color: #0f172a; }
            #calendario-citas .fc-button-primary { background: #4f46e5; border-color: #4f46e5; font-weight: 700; text-transform: uppercase; font-size: 10px; letter-spacing: .1em; }
            #calendario-citas .fc-button-primary:not(:disabled).fc-button-active,
            #calendario-citas .fc-button-primary:not(:disabled):active { background: #3730a3; border-color: #3730a3; }
    ...
    <div class="py-12 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm mb-6">
                <div class="flex flex-wrap gap-4 text-xs font-black uppercase tracking-widest text-slate-500 mb-6">
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-amber-500"></span> Pendiente</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-indigo-600"></span> Confirmada</span>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const calendario = new FullCalendar.Calendar(calendarioEl, {
                    ...
                    selectable: true,
                    businessHours: {
                        daysOfWeek: diasLaborales.map(d => d === 7 ? 0 : d),
                        startTime: '{{ config('salon.hora_apertura') }}',
                        endTime: '{{ config('salon.hora_cierre') }}',
                    },
                    hiddenDays: [0],
                    events: {
